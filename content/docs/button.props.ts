...
export const buttonProps: PropDef[] = [
  ...
  { name: "size", type: '"sm" | "default" | "lg" | "xl" | "icon-sm" | "icon" | "icon-lg" | "icon-xl" | "compact" | "icon-compact"', default: "from SizeProvider", description: "Step on the size ladder: 32px, 36px, 40px, 48px, with the `icon-*` squares to match. `compact` is the dense 28px control, and omitting the prop follows the site's size step (see the `--control-*` tokens in globals.css)." },
