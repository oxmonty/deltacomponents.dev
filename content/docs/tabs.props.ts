...
export const tabsProps: PropDef[] = [
  ...
  { name: "size", type: '"sm" | "default" | "lg"', default: '"default"', description: "A preset for the list's height and type size, which the triggers stretch to and inherit — the heights match Button's ladder (32, 36, 40px). `className=\"h-11 text-sm\"` on `TabsList` does the same job for a value the presets do not cover." },
